...
- 项目更名为 **Bursar**。本次只改产品名称：模块路径、二进制名、systemd unit 名、磁盘路径、`GPUOPS_*` 环境变量、`gpuops_controller_*` 指标名以及数据库标识符全部保持不变，已有部署、抓取配置与监控面板不受影响。默认平台名称与邮件 `from_name` 的变化只作用于新安装，已有安装仍沿用设置中已保存的值。
