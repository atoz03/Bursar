...
func (s *Server) RouterWeb() *gin.Engine {
	...
	admin.GET("/power-racks", s.requireSuperAdmin(), s.handleAdminPowerRacksOverview)
	admin.POST("/power-racks", s.requireSuperAdmin(), s.handleAdminPowerRackUpsert)
	admin.DELETE("/power-racks/:code", s.requireSuperAdmin(), s.handleAdminPowerRackDelete)
	admin.POST("/power-racks/:code/nodes", s.requireSuperAdmin(), s.handleAdminPowerRackNodeUpsert)
	admin.DELETE("/power-racks/:code/nodes/:node_id", s.requireSuperAdmin(), s.handleAdminPowerRackNodeDelete)
...
func (s *Server) handleAdminStatsMonthly(c *gin.Context) {
	...
	if restricted && len(visibleNodes) == 0 {
		c.JSON(http.StatusOK, gin.H{"from": formatRFC3339InBeijing(from), "to": formatRFC3339InBeijing(to), "rows": []UsageMonthlySummary{}, "has_more": false})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{"from": formatRFC3339InBeijing(from), "to": formatRFC3339InBeijing(to), "rows": rows, "has_more": hasMore})
}
